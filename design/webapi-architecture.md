...
- Live chat (`src/pages/ConversationThreadPage.tsx`) ports Vue's `ConversationThreadView.vue` state machine (canonical merge-by-id ordering, ack timeout, single HTTP-fallback guard, exponential reconnect with offline Retry, `online` recovery, `PolicyViolation` handling) onto React state. Each piece of mutable session state (thread, pending send, connection state, socket, timers, reconnect attempt/version counters) is mirrored into a `useRef` alongside its `useState`, so socket callbacks and `setTimeout` continuations always read the current value instead of the render that scheduled them. The effect that starts/tears down a thread's connection is keyed on the route's `id` param and sets a `disposed` ref true in its cleanup and false at the top of its body — this makes it correct both for a real conversation switch and for React 19 Strict Mode's mount→cleanup→mount double-invoke, which would otherwise leave two live sockets open. Tests (`ConversationThreadPage.test.tsx`, vitest + `@testing-library/react` + happy-dom, the first test tooling added to this client) drive a `FakeSocket` and fake timers through a real `createMemoryRouter`/`RouterProvider` rather than mocking the router, since route param changes there don't remount the component (matching Vue Router's behavior) and exercising the real router avoids reimplementing its reactivity in a mock.

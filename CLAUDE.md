...
Both variants share the same Postgres database and can run concurrently.

EF Core migrations (run from repo root; `dotnet-ef` is a local tool restored via `.config/dotnet-tools.json`; `AgoraFold.Mvc` is the only migrations startup project — `AgoraFold.RazorPages` doesn't need `Microsoft.EntityFrameworkCore.Design` for this reason):
...
- **Static files gotcha**: both variants' `Program.cs` register `app.UseStaticFiles()` and `app.MapStaticAssets()` — not redundant. `MapStaticAssets()` only serves assets known at build time from a manifest; it does NOT serve listing images uploaded at runtime under `wwwroot/uploads/listings/`, so `UseStaticFiles()` must stay for those to be servable.
- Each future variant (Blazor, Web API + JS, HTMX) will be its own project alongside `AgoraFold.Mvc`/`AgoraFold.RazorPages`, depending on the same `AgoraFold.Core` (services included), added to `AgoraFold.slnx`.

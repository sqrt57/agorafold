...
- **`AgoraFold.WebApi`** + **`AgoraFold.Vue`** + **`AgoraFold.React`** — the Web API + JS variants: a shared JSON API project plus separate SPA clients (Vite/Vue 3 + TypeScript, Vite/React + TypeScript — neither is a `.slnx` project) that consume it. The backend's auth/CORS/CSRF design (`Cors:JsClientOrigins` in `appsettings.json`) is shared across every Web API + JS frontend (Svelte, Angular, SolidJS still planned), not just one client — each new client's dev origin gets added to that array. `AgoraFold.React` is a straight feature-parity port of `AgoraFold.Vue` (same views, same `api/*.ts` client layer ported near-verbatim, same global `style.css`), just idiomatic React (Context+hooks instead of Pinia, React Router instead of vue-router). Full architecture notes: `design/webapi-architecture.md`.
